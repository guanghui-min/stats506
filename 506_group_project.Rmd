...
The coefficients are close to those when using **lme4**, and the regression using **lme4** has smaller BIC.
...
In this study, we applied the Linear Mixed Effects Model to Beijing Air Polution Dataset(pm2.5). After removing some of the outliers, we choose the variable "cbwd", abbreviation of "combined wind direction, as our grouping variable. "Cbwd" is treated as the random effect to fit the non-linear dataset using Mixed Effects Models.
...
When comparing the BICs with the Simple Linear Regression Model, the result shows that the Linear Mixed Effects Model fits better. 
...
Lastly, the method chosen from **REML** and **ML**(options in the two functions) to do regression should be taken into consideration.
