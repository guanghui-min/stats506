...
##Topic Introduction
Beijing's air pollution poses a big threat to the health of local residents. 
The PM 2.5 Index has been known as an important indication of the air quality. 
Exploring an effective way of predicting PM 2.5 Index will be an important step towards addressing the pollution. 
However, various factors such as dew point, temperature, pressure, etc. may influence PM 2.5 and the relationship between them remains unclear. 
Thus, it is challenging to accurately predict PM 2.5. To address this problem, linear mixed effects model is adopted to predict the response variable (ie. air quality) based on the numerous explanatory variables (meteorological records).
...
When fitting a complicated models with many parameters and observations, the data points might not be truly independent from each other.
For example, all observations from a meterological dataset could be collected on the same site. 
Therefore, mixed models were developed to overcome such problem and let us to be able to use all of our data. Mixed models are applied in many disciplines where multiple correlated measurements are made on each unit of interest.
The core of mixed model is that it incorporates fixed and random effects. The difference between fixed and random effects is that a fixed effect is an effect that is constant for a given population, 
but a random effect is an effect that varies for a given population.
...
#### Things to think
In this study, we apply linear mixed model to Beijing Air Polution Dataset(pm2.5). After removing some outliers we choose cbwd as a group variable,
which is an abbreviation of combined wind direction as random effect to fit the non-linear dataset using mixed effects models.
This group variable is selected using common sense and use plots to find random slopes. As can be seen from the above analyses,
...
Especially when we compare BIC with the simple linear regression, it shows that the linear mixed model fits better. 
However, we can conclude some additional results from this model for the development in future research using this method.
First, this method may not always work well if we don't have thorough knowledge of the dataset. For example,this model fits worse without cleansing the dataset removing outliers. 
Second, when there are many variables it will be more difficult to find which ones to put in the random slope part(in the model function, the part starting with (1|a+b+...+z).
Thus, we should pay more attention to model selection for applying Linear Mixed Model. Also, in R, when we use different packages, we should consider which model to use to fit the dataaset. 
In lmer function(from lme4 library) we used the suggested model but similarly, in lme function(from nlme library) we got the approximately the same result.
In addition, the method chosen from **REML** and **ML**(options in the two functions) to do regression should be taken into consideration.
